...
export const primevuePt: Record<string, Record<string, Cls | DynCls>> = {
  ...
  button: {
    root: (({ props }) => ({
      class: [
        ...
        // Size
        props['size'] === 'small' ? 'px-2.5 py-1 text-xs' : 'px-4 py-2 text-sm',
        // Severity
  ...
  dialog: {
    root: {
      class: 'bg-white rounded-xl shadow-2xl w-[480px] max-w-[92vw] mx-auto',
    },
    mask: {
      class:
        'fixed inset-0 bg-slate-900/45 z-50 flex items-center justify-center',
    },
    ...
    closeButton: {
      class:
        'p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer border-none bg-transparent',
    },
  ...
  inputtext: {
    root: {
      class:
        'w-full px-3 py-2 border border-slate-300 rounded-md text-sm text-slate-900 outline-none transition-colors focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 font-[inherit] bg-white',
    },
  },
  ...
  textarea: {
    root: {
      class:
        'w-full px-3 py-2 border border-slate-300 rounded-md text-sm text-slate-900 outline-none transition-colors focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 font-[inherit] resize-y bg-white',
    },
  },
  ...
  select: {
    root: {
      class:
        'flex items-center border border-slate-300 rounded-md text-sm bg-white cursor-pointer transition-colors focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20',
    },
    label: { class: 'flex-1 px-3 py-2 text-slate-900 text-sm' },
    dropdown: { class: 'px-2 text-slate-400 flex items-center' },
    panel: {
      class:
        'bg-white border border-slate-200 rounded-md shadow-lg mt-1 z-50 overflow-hidden',
    },
    listContainer: { class: 'overflow-y-auto max-h-60' },
    list: { class: 'py-1' },
    option: {
      class:
        'px-3 py-2 text-sm text-slate-900 hover:bg-indigo-50 cursor-pointer',
    },
  },
  ...
  checkbox: {
    ...
    box: {
      class: 'w-4 h-4 border-2 border-slate-300 rounded transition-colors',
    },
  ...
  toast: {
    ...
    message: (({ props }) => ({
      class: [
        ...
        props['message'] &&
        (props['message'] as Record<string, string>)['severity'] === 'success'
          ? 'bg-green-50 border-green-200'
          : props['message'] &&
              (props['message'] as Record<string, string>)['severity'] ===
                'error'
    ...
    closeButton: {
      class:
        'ml-auto p-0.5 rounded hover:bg-black/5 text-slate-400 hover:text-slate-600 border-none bg-transparent cursor-pointer',
    },
    closeIcon: { class: 'w-4 h-4' },
  },
}
